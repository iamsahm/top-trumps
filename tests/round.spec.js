...
describe("Round", () => {
    ...
    describe("addTurn", () => {
        it("should add a turn to the turns array", () => {
            const round = new Round();
            round.addTurn({ name: "test" }, { name: "test", syntax: 1 });
            expect(round.turns.length).toEqual(1);
        });
        it("should throw an error if the turn is not a tuple with two objects", () => {
            const round = new Round();
            expect(() => round.addTurn("test", "test")).toThrow(
                "Turn must be a tuple of two objects"
            );
        });
    });
    describe("defineWinner", () => {
        it("should return the player with the highest value for the given attribute", () => {
            const round = new Round();
            round.addTurn({ name: "test" }, { name: "test", syntax: 1 });
            round.addTurn({ name: "test2" }, { name: "test2", syntax: 2 });
            expect(round.defineWinner("syntax")).toEqual({ name: "test2" });
        });
        it("should return null if there is no winner", () => {
            const round = new Round();
            round.addTurn({ name: "test" }, { name: "test", syntax: 1 });
            round.addTurn({ name: "test2" }, { name: "test2", syntax: 1 });
            expect(round.defineWinner("syntax")).toEqual(null);
        });
